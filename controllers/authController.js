...
exports.signup = catchAsync(async (req, res, next) => {
    ...
    const verifyURL = `${req.protocol}://${req.get('host')}/api/users/verifyEmail/${verifyToken}`;
    //const message = `please verify your email by visiting this link ${verifyURL}`;
    try {
        new sendEmail({ email: newUser.email, name: newUser.name }, verifyURL).verifyEmail();
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
    ...
    if (!user) {
        return next(new AppError("there is no user wit that email", 404));
    }
    ...
    try {
        new sendEmail({ email: user.email, name: user.name }, resetURL).sendResetPassword();
